...
namespace XEngine {
    ...
    Accepter* Accepter::Create(Api::iTcpServer* server, const std::string& ip, const int port) {
        Accepter* ac = XPOOL_CREATE(s_AccepterPool, server, ip, port);
    ...
    void Accepter::OnCompleted(OVERLAPPEDEX* ex, const eCompletion type, const int code, const int size) {
        ...
        if (0 == code) {
            ...
            if (SOCKET_ERROR == setsockopt(_Ex._Socket, SOL_SOCKET, SO_UPDATE_ACCEPT_CONTEXT, (char*)&_Socket, sizeof(SOCKET))
                ...
                if (session) {
                    Tcper* pipe = Tcper::Create(session, ip, port, _Ex._Socket, false);
                    if (pipe) {
    ...
    Accepter::Accepter(Api::iTcpServer* server, const std::string& ip, const int port)
        : _Server(server), _ListenIP(ip), _ListenPort(port), _Socket(INVALID_SOCKET), _Ex(eCompletion::Accepted, this) {
    }
}
