...
namespace XEngine {
    ...
    void TimeWheel::KillTimer(Api::iTimer* timer, const int id, const UInt64 context) {
        TimeBase* base = FindTimerBase(timer, id, context);
    ...
    TimeBase* TimeWheel::CreateTimerBase(Api::iTimer* timer, const int id, const UInt64 context, int count, int interval, const char* file, const int line) {
    ...
    void TimeWheel::RemoveTimerBase(TimeBase* base, Api::iTimer* timer, const int id, const UInt64 context) {
        TIME_MAP::iterator itor = s_TimerMap.find(timer);
        if (itor != s_TimerMap.end()) {
    ...
    TimeBase* TimeWheel::FindTimerBase(Api::iTimer* timer, const int id, const UInt64 context) {
        TIME_MAP::iterator itor = s_TimerMap.find(timer);
        if (itor != s_TimerMap.end()) {
            TIMEBASE_MAP::iterator ifind = itor->second.find(TimerContext(id, context));
