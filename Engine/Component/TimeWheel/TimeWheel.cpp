...
namespace tcore {
    struct TimerContext : public iContext {
        const int _id;
        TimerContext(const int id, const iContext& context) : iContext(context), _id(id) {}
    ...
    typedef std::unordered_map<TimerContext, tbase*, tools::HashFunc<TimerContext>> TIMEBASE_MAP;
    typedef std::unordered_map<iTimer*, TIMEBASE_MAP> TIME_MAP;
    ...
    void timermgr::update() {
        ...
        int count = (int)(tick - last) / jiffies_interval;
    ...
    tbase* timermgr::createTimerBase(iTimer* timer, const int id, const iContext& context, int count, int64 interval, const char* file, const int line) {
        TIME_MAP::iterator itor = s_timer_map.find(timer);
        if (itor == s_timer_map.end()) {
            s_timer_map.insert(std::make_pair(timer, TIMEBASE_MAP()));
        ...
        itor->second.insert(std::make_pair(ct, base));
    ...
    void timermgr::removeTimerBase(tbase* base, iTimer* timer, const int id, const iContext& context) {
        TIME_MAP::iterator itor = s_timer_map.find(timer);
    ...
    tbase* timermgr::findTimerBase(iTimer* timer, const int id, const iContext& context) {
        TIME_MAP::iterator itor = s_timer_map.find(timer);
        if (itor != s_timer_map.end()) {
            TimerContext ct(id, context);
    ...
    void timermgr::startTimer(iTimer* timer, const int id, const iContext& context, int64 delay, int count, int64 interval, const char* file, const int line) {
    ...
    void timermgr::killTimer(iTimer* timer, const int id, const iContext& context) {
        tbase* base = findTimerBase(timer, id, context);
    ...
    void timermgr::pauseTimer(iTimer* timer, const int id, const iContext& context) {
        tbase* base = findTimerBase(timer, id, context);
        if (base == nullptr) {
    ...
    void timermgr::resumeTimer(iTimer* timer, const int id, const iContext& context) {
        tbase* base = findTimerBase(timer, id, context);
        if (base == nullptr) {
            //error(core::getInstance(), "where is timer base, timer %x, id %d", timer, id);
            return;
