...
int main(int argc, const char** args, const char** env) {
    ...
    SafeString::Replace(header_h_file_content, "{name}", static_parameter_map["module_name"]);
    SafeString::Replace(interface_h_file_content, "{name}", static_parameter_map["module_name"]);
    SafeString::Replace(template_h_file_content, "{name}", static_parameter_map["module_name"]);
    SafeString::Replace(template_cpp_file_content, "{name}", static_parameter_map["module_name"]);
    SafeString::Replace(dllexport_cpp_content, "{name}", static_parameter_map["module_name"]);
    SafeString::Replace(cmake_file_content, "{name}", static_parameter_map["module_name"]);
    SafeString::Replace(cmake_file_content, "{auther}", static_parameter_map["auther"]);
    SafeString::Replace(cmake_file_content, "{date}", SafeSystem::Time::GetCurrentTimeString());
    SafeString::Replace(cmake_file_content, "{group}", static_parameter_map["group"]);

    std::string path = SafeSystem::File::GetApplicationPath();
    auto itor = static_parameter_map.find("path");
    if (static_parameter_map.end() != itor) {
        path += "/" + itor->second;
    }

    std::string module_dir = path + "/" + static_parameter_map["module_name"];
    ...
    SafeSystem::File::DelFile(interface_dir + "/i" + static_parameter_map["module_name"] + ".h");
    XFile interface_h_file;
    if (!interface_h_file.Open(interface_dir + "/i" + static_parameter_map["module_name"] + ".h")) {
        XASSERT(false, "wtf");
        return false;
    }
    interface_h_file << interface_h_file_content;

    SafeSystem::File::DelFile(module_dir + "/" + static_parameter_map["module_name"] + ".h");
    XFile template_h_file;
    if (!template_h_file.Open(module_dir + "/" + static_parameter_map["module_name"] + ".h")) {
        XASSERT(false, "wtf");
        return false;
    }
    template_h_file << template_h_file_content;

    SafeSystem::File::DelFile(module_dir + "/" + static_parameter_map["module_name"] + ".cpp");
    XFile template_cpp_file;
    if (!template_cpp_file.Open(module_dir + "/" + static_parameter_map["module_name"] + ".cpp")) {
        XASSERT(false, "wtf");
        return false;
    }
    ...
    SafeString::Replace(add_subdirectory, "{name}", static_parameter_map["module_name"]);
