...
namespace XEngine {
    Quaternion Quaternion::Euler(const Vector3& v) {
        ...
        Quaternion q;
        q._W = (float)(cy * cp * cr + sy * sp * sr);
        q._X = (float)(cy * cp * sr - sy * sp * cr);
        q._Y = (float)(sy * cp * sr + cy * sp * cr);
        q._Z = (float)(sy * cp * cr - cy * sp * sr);
    ...
    Vector3 Quaternion::EulerAngles() const {
        ...
        if (std::abs(sinp) >= 1)
            angles.y = (float)std::copysign(MATH_PI / 2, sinp); // use 90 degrees if out of range
        else
