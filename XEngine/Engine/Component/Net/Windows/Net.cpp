...
namespace XEngine {
    ...
    void Net::EarlyUpdate(Api::iEngine* const engine) {
        ...
        } while (SafeSystem::Time::GetMicroSecond() - tick < 2000);
    }
    ...
    bool Net::LaunchTcpSession(Api::iTcpSession* session, const std::string& host, const UInt16 port, const int size) {
        SetLastError(0);
        DWORD ul = 1;
        int len = 0;
        LINGER linger = { 1,0 };
        ...
        if (INVALID_SOCKET == (socket = (int)WSASocket(AF_INET, SOCK_STREAM, IPPROTO_TCP, nullptr, 0, WSA_FLAG_OVERLAPPED))
        ...
        Tcper* pipe = Tcper::Create(session, ip, port, socket, true, size);
        if (nullptr == pipe) {
            SAFE_CLOSE_SOCKET(socket);
            session->OnTcpSessionEvent(Api::eTcpSessionEvent::Connectfaild);
            return false;
    ...
    bool Net::LaunchTcpServer(Api::iTcpServer* server, const std::string& ip, const UInt16 port, const int size) {
        Accepter* ac = Accepter::Create(server, ip, port, size);
        return ac != nullptr;
    }
}
