...
#define INVALID_FD -1
#define SUCCESS 0
#define KQUEUE_MAX_EVENT_COUNT 512
#define KQUEUE_DESC_COUNT 10240
...
namespace XEngine
{
    ...
    public:
        ...
        virtual void OnCompleter(Associat* at, const eCompletion type, const struct kevent& ev) = 0;
    };
...
#define DISCONNECTED_COUNT // DisconnectedCount(__FILE__, __LINE__)
    ...
    __forceinline int AddKqueueEvent(const int fd, const int16_t filter, void* udata)
    {
        struct kevent ev;
        EV_SET(&ev, fd, filter, EV_ADD, 0, 0, udata);
        return kevent(g_KqueueFd, &ev, 1, nullptr, 0, nullptr);
    }

    __forceinline int DeleteKqueueEvent(const int fd, const int16_t filter)
    {
        struct kevent ev;
    ...
    __forceinline int AddKqueueRead(const int fd, void* udata)
    {
        return AddKqueueEvent(fd, EVFILT_READ, udata);
    }

    __forceinline int AddKqueueWrite(const int fd, void* udata)
    {
        return AddKqueueEvent(fd, EVFILT_WRITE, udata);
    }

    __forceinline int AddKqueueReadWrite(const int fd, void* udata)
    {
        struct kevent evs[2];
        EV_SET(&evs[0], fd, EVFILT_READ, EV_ADD, 0, 0, udata);
        EV_SET(&evs[1], fd, EVFILT_WRITE, EV_ADD, 0, 0, udata);
        return kevent(g_KqueueFd, evs, 2, nullptr, 0, nullptr);
    }

    __forceinline void DeleteKqueueReadWrite(const int fd)
    {
        DeleteKqueueEvent(fd, EVFILT_READ);
        DeleteKqueueEvent(fd, EVFILT_WRITE);
    }
}
