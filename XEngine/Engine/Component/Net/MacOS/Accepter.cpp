...
namespace XEngine {
    ...
    Accepter* Accepter::Create(iTcpServer* server, const std::string& listenIp, const UInt16 listenPort, const int size) {
        ...
        if (ERROR_SOCKET == AddKqueueRead(sock, (void*)&(ac->_Associat))) {
    ...
    void Accepter::OnCompleter(Associat* at, const eCompletion type, const struct kevent& ev) {
        XASSERT(type == eCompletion::doAccept, "Accepter on completer error");
        if (ev.filter == EVFILT_READ && !(ev.flags & EV_ERROR)) {
            struct sockaddr_in addr;
            ...
            XERROR(g_Engine, "accept error %s", ev.flags & EV_ERROR ? strerror((int)ev.data) : strerror(errno));
        }
    }
