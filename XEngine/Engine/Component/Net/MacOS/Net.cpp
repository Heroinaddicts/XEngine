...
namespace XEngine {
    ...
    bool Net::Initialize(Api::iEngine* const engine) {
        if (INVALID_FD == (g_KqueueFd = kqueue())) {
            XERROR(g_Engine, "create kqueue fd error %s", strerror(errno));
            return false;
        }
    ...
    void Net::Run(void* context) {
        static struct kevent evs[KQUEUE_MAX_EVENT_COUNT];
        while (!_IsClose) {
            errno = 0;
            int count = 0;
            if ((count = kevent(g_KqueueFd, nullptr, 0, evs, KQUEUE_MAX_EVENT_COUNT, nullptr)) > 0) {
                for (int i = 0; i < count; i++) {
                    struct Associat* p = (struct Associat*)evs[i].udata;
